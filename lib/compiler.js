    ...
    if (this.options.coffee) {
      ...
      for(var i=0,l=this.buf.length; i < l; i++) {
        ...
        if (el === INDENT) {
          ...
          indent--;
        } else if (el.match(/__jadectx.unshift/)) {
        } else if (el.match(/__jadectx.shift/)) {
        } else {
          if (i > 0) {
            content += "\n";
          }
...
//    this.buf.push('buf.push(
  ...
  */
  ...
  visit: function(node){
    ...
    if (debug) {
      this.buf.push('__jadectx.unshift({lineno: ' + node.line
    ...
    if (debug) this.buf.push('__jadectx.shift();');
  },
  ...
  visitLiteral: function(node){
    ...
    if (this.options.rawdom) {
      ...
      this.buf.push(this.elRef() + ".innerHTML += '" + str + "'")
    } else {
      this.buffer(str);
    }
  },
    ...
    if (mixin.call) {
      ...
      if (block || attrs.length) {
        ...
        if (attrs.length) {
          ...
          if (this.options.coffee) {
            if (val.inherits) {
              this.buf.push('attributes: __jade.merge({' + val.buf + '}, attributes), escaped: __jade.merge(' + JSON.stringify(val.escaped) + ', escaped, true)');
            } else {
              this.buf.push('attributes: {' + val.buf + '}, escaped: ' + JSON.stringify(val.escaped));
            }
          } else {
            if (val.inherits) {
              this.buf.push('attributes: merge({' + val.buf + '}, attributes), escaped: merge(' + JSON.stringify(val.escaped) + ', escaped, true)');
            } else {
              this.buf.push('attributes: {' + val.buf + '}, escaped: ' + JSON.stringify(val.escaped));
            }
          }
      ...
      if (this.options.coffee) {
        if (args) {
          this.buf.push(name + ' = ((mixin_context, ' + args + ') ->');
        } else {
          this.buf.push(name + ' = ((mixin_context) ->');
        }
        this.buf.push(INDENT)
        this.buf.push('block = mixin_context.block; attributes = mixin_context.attributes || {}; escaped = mixin_context.escaped || {}');
      } else {
  ...
  visitTag: function(tag){
    ...
    if (this.options.rawdom) {
      this.buf.push(this.elRef() + ' = ' + this.elParentRef() + '.appendChild(document.createElement("' + name + '"))')
      if (tag.attrs.length) this.visitAttributes(tag.attrs);
      if (tag.code) this.visitCode(tag.code);
      this.escape = 'pre' == tag.name;
      this.visit(tag.block);
      this.elidx--;
      if (this.options.testHookPrettyPrint && !tag.isInline() && 'pre' != tag.name && !tag.canInline()) {
        // this is needed to communicate pretty-print entropy that isn't represented in the DOM
        // pretty-print calculates the linebreaks at COMPILE time even though they are inserted at EVAL time
        // Thus, code that generates flat text results in different pretty-print linebreaking than constant flat text
        // Those two cases can't be distinguished from the DOM data alone, and we need to wire through this hack for testing
        this.buf.push(this.elRef() + '.testHookPrettyPrint()');
      }
  ...
  visitText: function(text){
    ...
    if (this.options.rawdom) {
      this.buf.push("__jade.domAppendText(" + this.elRef() + ", '" + text + "')")
    } else {
      this.buffer(text);
    }
  },
  ...
  visitComment: function(comment){
    ...
    if (this.options.rawdom) {
      ...
      this.buf.push('__jade.domAppendComment(' + this.elRef() + ", '" + comment.val + "')");
    } else {
      if (this.pp) this.prettyIndent(1, true);
    ...
    if (this.options.coffee) {
      ...
      if (code.buffer) {
        if (this.options.rawdom) {
          if (code.escape) {
            ...
            this.buf.push('__jade.domAppendText(' + this.elRef() + ', (' + code.val.trimLeft() + '))')
          } else {
            // Unescaped text: Might have HTML elements inside and must be parsed
            this.buf.push('__jade.domAppendContent(' + this.elRef() + ', (' + code.val.trimLeft() + '))')
          }
        } else {
      ...
      if (code.buffer) {
        if (this.options.rawdom) {
          ...
          this.buf.push('var __val__ = ' + code.val.trimLeft());
  ...
  visitAttributes: function(attrs){
    ...
    if (this.options.rawdom) {
      ...
      for (var i=0, l=attrs.length; i < l; i++) {
        ...
        if (attr.name == 'class') {
          if (isConstant(attr.val)) {
            ...
            this.buf.push(this.elRef() + ".classList.add(" + arrayParamsSplat(classes) + ")");
          } else {
            // We can't assume that a computed value is a single class name.  It might be an array, so we call a helper
            this.buf.push('__jade.domSetClass(' + this.elRef() + ", (" + attr.val + "))");
          }
        } else if (attr.name == 'attributes') {
          this.buf.push("__jade.domSetAttributes(" + this.elRef() + ", (" + attr.val + "))");
        } else if (attr.name.indexOf('data') == 0) {
          this.buf.push("__jade.domSetDataAttribute(" + this.elRef() + ", '" + attr.name + "', (" + attr.val + "))");
        } else {
          this.buf.push("__jade.domSetAttribute(" + this.elRef() + ", '" + attr.name + "', (" + attr.val + "))");
        }
      }
    } else {
      var val = this.attrs(attrs);
      if (val.inherits) {
        if (this.options.coffee) {
          this.buf.push("buf.push __jade.attrs(__jade.merge({ " + val.buf + " }, attributes), __jade.merge(" + JSON.stringify(val.escaped) + ", escaped, true))");
        } else {
          this.buf.push("buf.push(attrs(merge({ " + val.buf + " }, attributes), merge(" + JSON.stringify(val.escaped) + ", escaped, true)));");
        }
        ...
        if (this.options.coffee) {
          this.buf.push("buf.push __jade.attrs(" + val.json + ", " + JSON.stringify(val.escaped) + ")");
        } else {
          this.buf.push("buf.push(attrs(" + val.json + ", " + JSON.stringify(val.escaped) + "));");
        }
      }
    }
  },
...
function getConstantClassListValues(val) {
  ...
  (function recurse(val) {
    console.log ("RECURSING OVER: ", val)
    var type = typeof val;
    if (val == null) {
      // add nothing
    } else if (type == 'number') {
